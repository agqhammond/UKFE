...
DDFExtract <- function(x, Plot = TRUE, main = NULL, Truncate = TRUE) {
  ...
  AMList <- list()
  for (i in 1:11) {
    AMList[[i]] <- suppressWarnings(AnnualStat(x, sum, Sliding = TRUE, Truncate = FALSE, N = Hours[i], na.rm = TRUE))[, 2]
  }
  RMED <- NULL
  for (i in 1:length(AMList)) {
    RMED[i] <- median(AMList[[i]])
  }
  ...
  GEVGF2 <- function(x, RP) {
    GEVGF(Lcv(x), LSkew(x), RP)
  }
  GF5 <- NULL
  for (i in 1:length(Hrs)) {
    GF5[i] <- GEVGF2(AMList[[i]], RP = 5)
  }
  GF10 <- NULL
  for (i in 1:length(Hrs)) {
    GF10[i] <- GEVGF2(AMList[[i]], RP = 10)
  }
  GF20 <- NULL
  for (i in 1:length(Hrs)) {
    GF20[i] <- GEVGF2(AMList[[i]], RP = 20)
  }
  GF50 <- NULL
  for (i in 1:length(Hrs)) {
    GF50[i] <- GEVGF2(AMList[[i]], RP = 50)
  }
  GF100 <- NULL
  for (i in 1:length(Hrs)) {
    GF100[i] <- GEVGF2(AMList[[i]], RP = 100)
  }
  GF200 <- NULL
  for (i in 1:length(Hrs)) {
    GF200[i] <- GEVGF2(AMList[[i]], RP = 200)
  }
  GF500 <- NULL
  for (i in 1:length(Hrs)) {
    GF500[i] <- GEVGF2(AMList[[i]], RP = 500)
  }
  GF1000 <- NULL
  for (i in 1:length(Hrs)) {
    GF1000[i] <- GEVGF2(AMList[[i]], RP = 1000)
  }
  ...
  ResDF <- data.frame(
    ModRMED, Mod5, Mod10, Mod20, Mod50, Mod100,
