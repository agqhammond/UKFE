...
#'@details The user can input a dataframe of dates (or POSIXct) and flow to return a plot of the flow duration curve for annual, winter and summer. Or a list of flow series' (vectors) can be applied for a plot comparing the individual flow duration curves
...
#'@param AddQs The is to add additional flows and associated horizontal plot lines to the plot. It should be a single numeric value or a vector, for example c(25, 75, 100).
...
#'@param ylab Label for the yaxis (character string). The default is "value",
#'@param xlab Label for the xaxis (character string). The default is "Time index".
#'
#'@examples
...
FlowSplit <- function(x, BaseQUpper = NULL, AdjUp = NULL, ylab = "Value", xlab = "Time index") {
  ...
  QOff <- BaseQUpper
  if(is.null(QOff)) {MeanFlow <- mean(x, na.rm = TRUE)}
  if(is.null(QOff) == FALSE) {MeanFlow <- QOff}
